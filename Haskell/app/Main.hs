...
import Assignment (bnfParser, hklCodeGen, validatator, getTime, getParserNames)
...
main = newQSemN 1 >>= \sem -> scotty 3000 $ do
        ...
        case parse bnfParser (unpack grammar) of
            ...
            Result _ bnf -> do
                (fs, result) <- liftIO . withLock sem $ do
                  ...
                  -- Get parser names directly from the grammar
                  let fs = getParserNames bnf

                  -- Compile the generated module and run selected parser
                  _ <- liftIO $ getCompiledFunctions outputFile
                  result <-
                      liftIO $
    ...
    post "/api/save" $ do
        ...
        case parse bnfParser (unpack grammar) of
            r@(Error _) -> jsonResponse [("error", show r)]
            Result _ bnf -> do
                saveFile <- liftIO . withLock sem $ do
                    -- Get timestamp and generate code
                    timestamp <- getTime
                    let code = hklCodeGen bnf
                    
                    -- Get first parser name for filename
                    let fs = getParserNames bnf
                    let firstParser = case fs of
                            (p:_) -> p
                            [] -> "output"
                    
                    -- Save file with timestamp comment
                    let saveFile = "output" </> (capitalizor firstParser ++ ".hs")
                    let savedCode = "-- " ++ timestamp ++ "\nmodule " ++ capitalizor firstParser ++ " where\n\n" ++ code
                    writeFile saveFile savedCode
                    return saveFile
